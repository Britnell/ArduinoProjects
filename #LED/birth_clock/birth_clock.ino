...
uint32_t fill_col_array( )
{
  for(uint8_t l=0; l<32; l++){
    ...
    else if(l < 17) { // 13 to 17       hours - purples
      r = map(l, 12,16, 5, 50);
      g = 0;
      b = map(l, 12,16, 5, 30 );
    }
    else if(l < 22) { // 18 to 22       days - blue  1.5 days
      r = 0;
      g = 0;
      b = map(l, 17,21, 8, 80 );
    }
    else if(l < 26){ // 23 to 26      1.5months - thiel
      r = 0;
      g = map(l, 22,25, 8, 50 );
      b = map(l, 22,25, 8, 50 );
    }
    else {  // 26 to rest years     1 year - white
      r = map(l, 26,32, 20,100 );
      g = map(l, 26,32,  8,80 );
      b = map(l, 26,32,  8,80 );
    }
    colors[l] = neoStrip.Color(r, g, b);
    neoStrip.setPixelColor(l, colors[l] );
    // eo for
  }
  neoStrip.show();
  // Eo func
...
int expon(int x, int imin, int imax, int omin, int omax ){
  float prop = 1.0 * (x-imin) / (imax-imin);
  return int(omin + ( prop * prop * (omax-omin) ));
}
...
void setup() {
  ...
  _D while(millis()-stopwatch<4000 && !setup_mode){
