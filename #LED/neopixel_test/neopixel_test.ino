...
void loop() {
  ...
  //the brush moves along the strip, leaving a colorful rainbow trail
	for(loopcounter = 0; loopcounter<duration; loopcounter++) 
	{
		...
		{
			initialized = true;
			pixelbrush.setSpeed(500/slow); //brush moving speed 
			pixelbrush.setFadeSpeed(90/slow);
	...
	for(loopcounter = 0; loopcounter<duration; loopcounter++) 
	{
		...
		{
			initialized = true;
			pixelbrush.setSpeed(600/slow/slow);
		...
		pixelbrush.setFadeSpeed((random(100) + 150)/slow); //set a new fadespeed with some randomness
  ...
  //brush set to random positions and painting a fading star
	for(loopcounter = 0; loopcounter<duration; loopcounter++) 
	{
		...
		if (rand() % 1000 == 0) //at a random interval, move the brush to paint a new pixel (brush only paints a new pixel once)
		{
			brushcolor.h = rand();
			...
			pixelbrush.setFadeSpeed((random(10)+5)/slow); //set random fade speed, minimum of 5
		}
...
//				pixelbrush.setSpeed(900/slow); //moving speed
//				pixelbrush.setColor(brushcolor);
     ...
     pixelcanvas.clear(); //clear the canvas
		...
		for(loopcounter = 0; loopcounter<duration; loopcounter++) 
		{
			...
			if (initialized == false)
			{
				...
				pixelbrush.setSpeed((random(150) + 150)/slow); //random movement speed
				pixelbrush.setFadeSpeed((random(10) + 20)/slow); //set random fading speed
				...
				pixelbrush2.setSpeed((random(150) + 150)/slow);
				pixelbrush2.setFadeSpeed((random(10) + 20)/slow);
				pixelbrush2.setColor(brushcolor);
...
//				pixelbrush.setColor(brushcolor);
//				pixelbrush.setSpeed(brushspeed/slow);
//				pixelbrush.setFadeSpeed(250/slow); //fast fading (255 is max.)
...
//				brushcolor.s = 250;
//				brushcolor.v = 150;
//				pixelbrush2.setSpeed(brushspeed/slow);
//				pixelbrush2.setFadeSpeed(220/slow);
...
//				brushcolor.s = 255;
//				brushcolor.v = 100;
//				pixelbrush3.setSpeed(brushspeed/slow);
//				pixelbrush3.setFadeSpeed(190/slow);
	...
	while(true) //create a loop with two additional brushes (are deleted automatically once the loop finishes)
	{
		...
		for(loopcounter = 0; loopcounter<duration; loopcounter++) 
		{
			...
			if (initialized == false) //initialize the brushes
			{
				...
				//first brush
				pixelbrush.setSpeed(-10); //zero initial speed
				pixelbrush.setFadeSpeed(150/slow);
				pixelbrush.setFadeout(true);
				pixelbrush.setColor(brushcolor);
...
//				pixelbrush2.setFadeSpeed(190/slow);
//				pixelbrush2.setFadeout(true);
//				pixelbrush2.setColor(brushcolor);
...
//				pixelbrush3.setFadeSpeed(220/slow);
//				pixelbrush3.setFadeout(true);
//				pixelbrush3.setColor(brushcolor);
	...
	while(true) //create a loop with two additional brushes (are deleted automatically once the loop finishes)
	{
		...
		for(loopcounter = 0; loopcounter<duration; loopcounter++) 
		{
			...
			if (initialized == false) //initialize the brushes
			{
				...
				pixelbrush.setSpeed(-750/slow);
				pixelbrush.setSpeedlimit(1000);
				pixelbrush.setFadeSpeed((random(80) + 50)/slow);
				pixelbrush.setFadeout(true);
				...
				brushcolor.h = 160;
				pixelbrush2.setSpeed(600/slow);
				pixelbrush2.setSpeedlimit(1000/slow);
				pixelbrush2.setFadeSpeed((random(80) + 50)/slow);
				pixelbrush2.setFadeout(true);
