...
void MMA::init()
{
  ...
  MMA7660_I2C_SEND(0x08,0x20);    // 0x00-no debounce, 000-tap detect sample rate
...
uint8_t *MMA::readData( uint8_t ADRS, uint8_t bytes ){
  uint8_t read_data[bytes];
  
  Wire.beginTransmission(MMA7660_ADDRESS);
  Wire.write(ADRS);  // register to read
  Wire.endTransmission();
  Wire.requestFrom(MMA7660_ADDRESS, bytes); // read a byte
