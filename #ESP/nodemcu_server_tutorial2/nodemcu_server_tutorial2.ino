      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            // Adding a slider
            client.println("<p><label for=\"fader\">Volume</label>");
            client.println("<input type=\"range\" min=\"0\" max=\"10\" value=\"4\" id=\"slider\" step=\"2\" onChange=\"slideFunc()\" ></p>");
            client.println("<script  type=\"text/javascript\"> function slideFunc() { ");
            client.println("X=document.getElementById('slider').value;  ");
            client.println("window.location.href='/slider/'+X.toString();");
