...
void setup() {
  ...
  // turn on brush
  pixelbrush.setSpeed(230);     // 4096 = one pixel per update
  pixelbrush.setFadeSpeed(40);  // 255 max
  pixelbrush.setFadein(true);
  pixelbrush.setFadeout(true);
  pixelbrush.setBounce(true);
    HSV brushcolor;
  brushcolor.h = 88;  // 0-255   ;  0=red
  brushcolor.s = 100; // 255 = 100%
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /5/on") >= 0) {
              ...
              slideVar = val.toInt();
              for(int x=0; x<PIXELS; x++) {
                neopixels.setPixelColor( x, neopixels.Color(0, slideVar>>4, slideVar) );
              }
              neopixels.show();
            }
